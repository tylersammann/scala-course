...
object FunSets {
   ...
   */
  def singletonSet(elem: Int): Set = (argElem: Int) => elem == argElem
  ...
  def map(s: Set, f: Int => Int): Set = {
    def iter(index: Int, elem: Int): Boolean = {
      if (index > bound) false
      else if (contains(s,index) && f(index) == elem) true
      else iter(index+1, elem)
    }
    (elem: Int) => iter(-bound, elem)
  }
